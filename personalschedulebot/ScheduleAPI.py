...
async def get_possible_days() -> List[ElectiveLessonDay]:
    result: httpx.Response = await make_api_get_request("/elective/days", {})
    return [ElectiveLessonDay(item) for item in result.json()]


async def get_possible_lessons(elective_day_id: int, partial_name: str) -> List[ElectiveLesson]:
    result: httpx.Response = await make_api_get_request("/elective/lessons", {
...
async def get_user_elective_lessons(telegram_id: int) -> List[ElectiveLesson]:
    result: httpx.Response = await make_api_get_request("/elective", {
        "TelegramId": telegram_id,
    })
    return [ElectiveLesson(item) for item in result.json()]


async def create_user_elective_lesson(telegram_id: int, lesson_id: int) -> bool:
    result: httpx.Response = await make_api_post_request("/elective", {}, {
        "TelegramId": telegram_id,
...
async def delete_user_elective_lessons(telegram_id: int, lesson_id: int) -> bool:
    result: httpx.Response = await make_api_delete_request("/elective", {
        "TelegramId": telegram_id,
        "LessonId": lesson_id,
    })
